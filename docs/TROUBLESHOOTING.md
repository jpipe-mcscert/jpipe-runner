...
| `graphviz`   | CLI tools not found or diagram generation fails                        | **Install CLI (Debian/Ubuntu):**<br>`sudo apt-get install graphviz libgraphviz-dev pkg-config build-essential python3-dev`<br>**Install CLI (macOS):**<br>`brew install graphviz`<br>**Poetry install (macOS):** pygraphviz compiles a C extension and needs the Graphviz headers exposed to the compiler:<br>`CFLAGS="-I$(brew --prefix graphviz)/include" LDFLAGS="-L$(brew --prefix graphviz)/lib" poetry install`<br>To avoid setting these every time, add them to `~/.zshrc`:<br>`export CFLAGS="-I$(brew --prefix graphviz)/include"`<br>`export LDFLAGS="-L$(brew --prefix graphviz)/lib"`<br>**Python Bindings (for diagram export):** `pip install pygraphviz`<br>Note: the `graphviz` PyPI package is different and does not support this export path.<br>**Verify:** `dot -V` | **Install CLI:**<br>Download from [https://graphviz.org/download/](https://graphviz.org/download/)<br>Add Graphviz `bin` directory to your `PATH`.<br>**Python Bindings (for diagram export):** `pip install pygraphviz`<br>Note: the `graphviz` PyPI package is different and does not support this export path.<br>**Verify:** `dot -V` |
